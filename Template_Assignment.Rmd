...
```{r visualise_map}
# Haal Nederland-provincies binnen als sf-object
nl_prov <- ne_states(country = "Netherlands", returnclass = "sf")

# Maak de kolom met korte provincie-namen in je data
data_2021 <- data_combined %>%
  filter(Year == "2021") %>%
  mutate(provincie_kort = gsub(" \\(PV\\)", "", Regio.s))

# Combineer kaart en tekort-data
kaart_met_tekort <- nl_prov %>%
  left_join(data_2021, by = c("name" = "provincie_kort"))

# Plot de kaart
ggplot(kaart_met_tekort) +
  geom_sf(aes(fill = Tekort), color = "black", size = 0.2) +
  scale_fill_gradient(low = "blue", high = "red", name = "Woningtekort") +
  labs(
    title = "Woningtekort per provincie (2021)",
    subtitle = "Hoe roder, hoe groter het tekort",
