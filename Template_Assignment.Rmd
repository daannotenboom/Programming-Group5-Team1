...
data_map <- data_combined %>%
  filter(Year == "2021") %>%
  mutate(
    provincie_kort = gsub(" \\(PV\\)", "", Regio.s),
    provincie_kort = ifelse(provincie_kort == "Fryslân", "Friesland", provincie_kort),
    Netto_Migratie_Perc_2021 = ,
  )

# Voeg de berekende waarde toe aan de kaart
kaart_met_migratie <- nl_prov_alleen_prov %>%
  left_join(data_map, by = c("name" = "provincie_kort"))

# Plot de kaart
ggplot(kaart_met_migratie) +
  geom_sf(aes(fill = Netto_Migratie_Perc_2021), color = "black", size = 0.5) +
  scale_fill_gradient(
    low = "red",
    high = "white",
    name = "Netto migratie %",
  ...
  labs(
    title = "Netto migratie per provincie (2021)",
    subtitle = "Als percentage van de bevolking in 2021"
  ) +
